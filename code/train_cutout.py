...
train_data = mydataset(cifar100_train, train=True, method='cutout')
...
for i in tqdm(range(epoch)):
    ...
    torch.save(sim_model.state_dict(), f'weight/cutout/cutout_epoch{i+1}.pth')  # 保存参数
    writer.add_scalars('cutout_loss', {'train': train_loss_epoch,
                                       'test': test_loss_epoch}, i+1)
    writer.add_scalars('cutout_acc', {'train': train_acc,
                                      'test': test_acc}, i+1)
