...
train_data = mydataset(cifar100_train, train=True, method='cutmix')
...
for i in tqdm(range(epoch)):
    ...
    torch.save(sim_model.state_dict(), f'weight/cutmix/cutmix_epoch{i + 1}.pth')  # 保存参数
    writer.add_scalars('cutmix_loss', {'train': train_loss_epoch,
                                       'test': test_loss_epoch}, i+1)
    writer.add_scalars('cutmix_acc', {'train': train_acc,
                                      'test': test_acc}, i+1)
