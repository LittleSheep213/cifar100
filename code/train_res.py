...
for i in tqdm(range(epoch)):
    ...
    torch.save(res_model.state_dict(), f'weight/res/res_epoch{i + 1}.pth')  # 保存参数
    writer.add_scalars('res_loss', {'train': train_loss_epoch,
                                    'test': test_loss_epoch}, i+1)
    writer.add_scalars('res_acc', {'train': train_acc,
                                   'test': test_acc}, i+1)
